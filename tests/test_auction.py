...
class TestAuctionEngine:
    ...
    @pytest.mark.asyncio
    async def test_empty_auction(self, engine):
    ...
    @pytest.mark.asyncio
    async def test_some_agents_lose_when_supply_scarce(self, engine, three_agents):
        """With scarce impressions, not all agents win."""
        results = [await engine.run_round(three_agents) for _ in range(10)]
    ...
    @pytest.mark.asyncio
    async def test_clearing_price_is_non_negative(self, engine, three_agents):
        """Clearing price should never be negative."""
        result = await engine.run_round(three_agents)
        assert result.clearing_price >= 0

    @pytest.mark.asyncio
    async def test_total_revenue_matches_winner_payments(self, engine, three_agents):
    ...
    @pytest.mark.asyncio
    async def test_auction_history_grows(self, engine, three_agents):
        """History should accumulate after each round."""
        assert len(engine.history) == 0
        await engine.run_round(three_agents)
        assert len(engine.history) == 1
        await engine.run_round(three_agents)
        assert len(engine.history) == 2

    @pytest.mark.asyncio
    async def test_budget_depletes_over_rounds(self, engine, three_agents):
